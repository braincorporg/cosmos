...
The id of Author should always follow this format : Brett J. Kagan become BJKagan.
You absolutly need to create a paper label, first extract the title of the paper.
Case Sheet:
$ctext
"""
...
"""
instructions="""
You are a mastermind of CypherQuery and Knowledge graph. Your role is to assist researcher reading paper and searching for informations. 
Below is the actual structure of the knowledge graph.
0. ALWAYS FINISH THE OUTPUT. Never send partial responses
...
The output should look like :
```output
    MATCH (author:Author {id: "BJKagan"})-[:WRITES]->(paper:Paper)
    RETURN paper
```
...
def generate_cypher(json_obj):
    ...
    with open("cyphers.txt", "w" , encoding='utf-8') as outfile:
...
def generateQuery(user_query):
    ...
    query=query.replace("```output","").replace("```","")
    print(query)
    return query
...
def ChatGPT(user_query):
    query=generateQuery(user_query)
    result = execute_query(gds, query)
    response = client.chat.completions.create(
        model= gpt4_model,
        max_tokens=4096,
        temperature=0,
...
You are a Researcher assistant, you are provided with result from knowledge graph queries that will help you respond to the researcher query.
researcher query : {user_query}

retrived content : {result}
...
ingestion("C:/Users/korn2/Desktop/BCtools/vitro.pdf")
folders = {
    "people_profiles": research_paper_prompt_template,
    }
ingestion_pipeline(folders)
response = ChatGPT("Find paper that discuss about biocomputing")
